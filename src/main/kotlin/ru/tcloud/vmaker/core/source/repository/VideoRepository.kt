...
class VideoRepository(
    ...
    override suspend fun getNotUsage(src: MediaSource): List<Video> {
        val query = Query.query(Criteria.where("src").`is`(src).andOperator(
            Criteria().orOperator(
    ...
    override suspend fun update(entity: Video): Video {
        return mongoTemplate.save(entity)
            .retryWhen(retrySpec)
            .awaitFirst()
    }

    override suspend fun add(entities: Collection<Video>): List<Video> {
